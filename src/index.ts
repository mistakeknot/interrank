...
function resolveRefreshMs(argv: string[]): number {
  const arg =
    parseArgValue(argv, "--refresh-ms") ??
    process.env.AGMODB_SNAPSHOT_REFRESH_MS;
...
async function main() {
    ...
    "list_models",
    {
      ...
      inputSchema: {
        query: z
          .string()
          .optional()
          .describe("Case-insensitive search on name/slug/provider."),
        provider: z
          .string()
          .optional()
          .describe("Provider slug or provider name."),
        metricKey: z
          .string()
          .optional()
          .describe("Metric key to sort by (default: agmobench)."),
  ...
  server.registerTool(
    ...
    {
      description:
        "Get a model by slug, including all metric values and prediction flags.",
      inputSchema: {
        slug: z.string().min(1),
      },
    },
  ...
  server.registerTool(
    ...
    async ({ query, category, source: benchmarkSource, limit }) => {
      ...
      if (query) {
        ...
        items = items.filter((benchmark) => {
          return (
            ...
            (benchmark.relevantUseCases ?? []).some((uc) =>
      ...
      if (category) {
        ...
        items = items.filter(
          (benchmark) => benchmark.category.toLowerCase() === c,
        );
      }
      ...
      if (benchmarkSource) {
        ...
        items = items.filter(
          (benchmark) => benchmark.source.toLowerCase() === s,
        );
      }
    ...
    "recommend_benchmarks",
    {
      ...
      inputSchema: {
        task: z
          ...
          .min(1)
          .describe(
            "Natural-language task description, e.g. 'customer support chatbot' or 'code review agent'.",
          ),
        categories: z
          .array(z.string())
          .optional()
          .describe("Optional category filter (e.g. ['coding', 'agents'])."),
        limit: z
          .number()
          .int()
          .min(1)
          .max(MAX_LIMIT)
          .optional()
          .describe("Max benchmarks to return (default: 10)."),
      },
    },
    ...
    "leaderboard",
    {
      ...
      inputSchema: {
        ...
        includePredicted: z
          .boolean()
          .optional()
          .describe("Include BenchPress-predicted cells (default: true)."),
        limit: z.number().int().min(1).max(MAX_LIMIT).optional(),
      },
    },
    ...
    "compare_models",
    {
      ...
      inputSchema: {
        ...
        benchmarkKeys: z
          ...
          .optional()
          .describe(
            "Specific benchmark keys to compare on. If omitted, uses all benchmarks with data.",
          ),
        category: z
          .string()
          .optional()
          .describe("Filter benchmarks by category (e.g. 'coding')."),
        limit: z
          .number()
          .int()
          .min(1)
          .max(MAX_LIMIT)
          .optional()
          .describe("Max benchmarks in the matrix."),
      },
    },
  ...
  server.registerTool(
    "domain_leaderboard",
    {
      ...
      inputSchema: {
        domain: z
          ...
          .describe("AgMoBench domain to rank by."),
        provider: z
          .string()
          .optional()
          .describe("Filter by provider slug or name."),
        includePredicted: z
          .boolean()
          .optional()
          .describe("Include BenchPress-predicted cells (default: true)."),
    ...
    async ({ domain, provider, includePredicted, limit }) => {
      ...
      if (!metric) {
        throw new Error(
          `Domain metric not found: ${metricKey}. Valid domains: ${VALID_DOMAINS.join(", ")}`,
        );
      }
  ...
  server.registerTool(
    "cost_leaderboard",
    {
      ...
      inputSchema: {
        metricKey: z
          ...
          .optional()
          .describe(
            "Metric key to rank efficiency for. Required if domain is not provided. Mutually exclusive with domain.",
          ),
        domain: z
          .enum([
            "overall",
          ...
          .optional()
          .describe(
            "AgMoBench domain. Required if metricKey is not provided. Mutually exclusive with metricKey.",
          ),
        costMetric: z
          .string()
          .optional()
          .describe("Cost metric key (default: blendedPricePerM)."),
        provider: z
          .string()
          .optional()
          .describe("Filter by provider slug or name."),
    ...
    async ({
      ...
      const resolvedMetricKey = domain
        ? DOMAIN_METRIC_KEYS[domain]
        : metricKey!;
      ...
      const ranked = withData.map((model) => {
        ...
        const efficiencyRatio =
          Math.round((benchmarkScore / costValue) * 100) / 100;
    ...
    "resolve_routing_name",
    {
      ...
      inputSchema: {
        name: z
          ...
          .min(1)
          .describe(
            "Routing name, alias, or slug to resolve (e.g. 'opus', 'claude-sonnet', 'gpt-5.3-codex-spark').",
          ),
        domains: z
          .array(
            z.enum([
          ...
          .optional()
          .describe(
            "AgMoBench domains to include in the response (default: all).",
          ),
      },
    },
    async ({ name, domains }) => {
    ...
    "routing_compare",
    {
      ...
      inputSchema: {
        names: z
          ...
          .max(10)
          .describe(
            "Routing names to compare (e.g. ['haiku', 'sonnet', 'opus']).",
          ),
        domains: z
          .array(
            z.enum([
          ...
          .optional()
          .describe("AgMoBench domains to compare (default: all)."),
      },
    },
    async ({ names, domains }) => {
      const state = await store.get();
      ...
      for (const name of names) {
        const family =
          state.indexes.familyByName.get(name.trim().toLowerCase()) ?? null;
        ...
        for (const domain of domainKeys) {
          const metricKey = DOMAIN_METRIC_KEYS[domain];
          domainScores[domain] =
            model && metricKey ? getMetricValue(model, metricKey) : null;
        }
        ...
        entries.push({
          ...
          speed: {
            tokensPerSec: model
              ? getMetricValue(model, "outputTokensPerSec")
              : null,
      ...
      for (const entry of entries) {
        if (
          entry.speed.tokensPerSec != null &&
          entry.speed.tokensPerSec > fastestSpeed
        ) {
      ...
      return jsonContent({
        ...
        items: families.map((f) => {
          ...
          return {
            ...
            aliases: f.aliases,
            resolved: primaryModel != null,
            agmobench: primaryModel
              ? getMetricValue(primaryModel, "agmobench")
              : null,
            price: primaryModel
              ? getMetricValue(primaryModel, "blendedPricePerM")
              : null,
          };
        }),
      });
  ...
  console.error(
    `interrank MCP server started — source: ${sourceLabel(source)}`,
  );
  store.get().catch((err) => {
    console.error(
